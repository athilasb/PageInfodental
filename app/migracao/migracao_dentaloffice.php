			...
			if(empty($erro)) {
				...
				if($sistema=="dentaloffice") {
					...
					foreach($pacientesCSV as $p) {
						...
						list($Data_do_Cadastro,$Nome,$Telefone,$Celular,$Telefone_Comercial,$Email,$Data_de_Nascimento,$CPF,$RG,$Sexo, $enderecoes, $bairroes, $numero, $complemento, $cidaderes, $ufres, $cepres, $enderecocom, $bairrocom, $cidadecom,	$ufcom,	$cepcom, $convenionum, $nomepai, $profpai, $nomemae, $profmae, $indicadornum, $responsavelnum, $profissaonum, $empresanum, $localnascimento, $matricula, $estadocivil, $dentistanum)=explode(";",$p);

					


						$Especialidades=$Titular=$Convenio=$Plano=$Número_do_Plano=$Validade_do_Plano=$Responsavel=$Nome_do_Pai=$Nome_da_Mae='';

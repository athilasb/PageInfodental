<?php
	...
	# Consulta $infoConta e $usr #
		...
		$usr='';
		if(isset($request->id_colaborador) and is_numeric($request->id_colaborador)) {
			$sql->consult($_p."colaboradores","id,nome","where id=".$request->id_colaborador);
			if($sql->rows) $usr=mysqli_fetch_object($sql->mysqry);
		}
	...
	if (isset($request->token) and $request->token == $token) {
		...
		# Instanciamentos #
			$attr = array('prefixo' => $_p, 'usr' => (object) is_object($usr) ? $usr : array('id' => 0));
		...
		if (is_object($infoConta)) {
			...
			if (isset($request->method)) {
				...
				# Gera PDF via domPDF de evoluções #
					if ($request->method == 'generatePDF') {
						...
						// capta dados
							...
							$assinatura = '';
							if(is_object($evolucao) and isset($request->id_assinatura) and is_numeric($request->id_assinatura) and $request->id_assinatura>0) {
								$sql->consult($_p."pacientes_evolucoes_assinaturas","*","where id=$request->id_assinatura and id_evolucao=$evolucao->id and lixo=0");
						...
						if (empty($erro)) {
							...
							$sql->consult($_p . "colaboradores", "nome,cro,uf_cro", "where id=$evolucao->id_profissional");
							if ($sql->rows) {
							...
							$divDadosClinica = '<div style="margin-top:2.5rem; style="text-align: left"">
													<table>
														<tr>
							...
							// se for para assinar
								...
								if(is_object($assinatura)) {
									$divAssinatura = '<div style="margin-top:2.5rem; style="text-align: left"">
														<center><img src="'.$assinatura->assinatura.'"></center>
														Assinado em <b>'.date('d/m/Y H:i').'</b> por <b>'.utf8_encode($paciente->nome).'</b><br />
														CPF: <b>'.$assinatura->cpf.'</b>
														Data de Nascimento: <b>'.date('d/m/Y',strtotime($assinatura->data_nascimento)).'</b>;
														</div>';
								}
							...
							if ($evolucao->id_tipo == 1) {
								...
								$dirEvolucao = "arqs/pacientes/anamneses/";

								if(is_object($assinatura)) $dirEvolucao.="assinados/";
				...
				# Assina PDF (insere image base64 no PDF) 
					else if ($request->method == "signPDF") {
					}

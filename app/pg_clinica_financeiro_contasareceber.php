...
$prefiltroDatas = isset($_POST['prefiltro_datas']) ? $_POST['prefiltro_datas'] : 7;
$data_inicial_filtro = isset($_GET['data_i']) ? $_GET['data_i'] : date('Y-m-d');
$data_final_filtro =  isset($_GET['data_f']) ? $_GET['data_f'] : date('Y-m-d', strtotime("+$prefiltroDatas days"));
...
$where = "WHERE (data_vencimento>='$data_inicial_filtro' AND data_vencimento<='$data_final_filtro') AND  id_fusao=0 and lixo=0 order by data_emissao desc, id asc";
...
$sql->consult($_p . "financeiro_fluxo", "*", "WHERE (data_vencimento>='$data_inicial_filtro' AND data_vencimento<='$data_final_filtro') AND id_registro IN (" . implode(",", $pagamentosIDs) . ") and lixo=0 order by data_vencimento asc");
if ($sql->rows) {
	while ($x = mysqli_fetch_object($sql->mysqry)) {
...
foreach ($registros as $x) {
	...
	if (isset($_baixas[$x->id])) {
		...
		foreach ($_baixas[$x->id] as $v) {
			if ($v->lixo == 0 && ($v->data_vencimento >= $data_inicial_filtro && $v->data_vencimento <= $data_final_filtro)) {
		...
		<section class="filter">
			...
			<div class="filter-group">
				...
				<div class="button-group">
					<a href="#" class="button active">7 dias</a>
					<a href="#" class="button">30 dias</a>
					<a href="#" class="button">60 dias</a>
					<a href="#" class="button">90 dias</a>
					<a href="#" class="button">ano</a>
				</div>
			</div>
		</section>
		<section class="grid">
			<div class="box">
				<section class="filter" style="margin-bottom:0;">
					<div class="filter-group">
						<div class="filter-title">
							<p>Total</p>
							<h2><strong>R$ <?= number_format($valor['valorTotal'], 2, ",", "."); ?></strong></h2>
						</div>
						<div class="filter-title">
							<p>A receber</p>
							<h2 style="color:var(--cinza4)">R$ <?= number_format(($valor['aReceber']), 2, ",", "."); ?></h2>
						</div>
						<div class="filter-title">
							<p>A definir pagamento</p>
							<h2 style="color:var(--laranja)">R$ <?= number_format($valor['definirPagamento'], 2, ",", "."); ?></h2>
						</div>
						<div class="filter-title">
							<p>Recebido</p>
							<h2 style="color:var(--verde)">R$ <?= number_format($valor['valorRecebido'], 2, ",", "."); ?></h2>
						</div>
						<div class="filter-title">
							<p>Vencido</p>
							<h2 style="color:var(--vermelho)">R$ <?= number_format($valor['valoresVencido'], 2, ",", "."); ?></h2>
						</div>
					</div>
					<div class="filter-group">
						<a href="#" class="button"><i class="iconify" data-icon="fluent:chevron-down-24-regular"></i> <span>Gráficos</span></a>
					</div>
				</section>
			</div>
	...
	function populaPagamentos() {
		...
		for (let pagamento of _pagamentos) {
			...
			if (baixas) {
				for (let b in baixas) {
					let baix = baixas[b];
					if (baix.data_vencimento >= data_inicial && baix.data_vencimento <= data_final) {
						valorEmJogo += parseFloat(baix.valor)
						valorMulta += parseFloat(baix.valor_multa)
						valorJuros += parseFloat(baix.valor_juros)

					}
				}
			} else {
				valorEmJogo = parseFloat(pagamento.valor)
			}
			//return
			...
			$('#list-payments tbody').append(`
				<tr data-idPagamento='${pagamento.id}'>
					...
					<td><strong>R$ ${number_format(valorEmJogo,2,',','.')}</strong></td>
					<td style="font-size:0.813em; line-height:1.2;">Multa: R$ ${number_format(valorMulta,2,',','.')}<br />Juros: R$ ${number_format(valorJuros,2,',','.')}</td>
					<td style="font-size:1.75rem;">

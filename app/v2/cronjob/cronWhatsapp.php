<?php
	...
	# Envia confirmacao de 24-48h para agendamentos realizados a mais de 7 dias (id_tipo=1)
		echo "<h1>Lembrete de 24h-48h</h1>";
		$dataInicio = date('Y-m-d H:i:s',strtotime(date('Y-m-d H:i:s')." + 24 hours"));
		...
		//echo $dataInicio."<br />".$dataFim." $sql->rows";die();
		if($sql->rows) {
	...
	# Envia confirmacao de 3h de antecedencia para agendamentos confirmados (id_tipo=2)
		...
		echo $dataInicio."<br />".$dataFim." $sql->rows<BR><BR>";
		if($sql->rows) {
			while($x=mysqli_fetch_object($sql->mysqry)) {

...
<section id="conteudo">
	...
	$_marcas=array();
	$sql->consult($_p."veiculos_marcas","*","where lixo=0 order by titulo asc");
	if($sql->rows) {
	...
	if(isset($_GET['form'])) {
		$cnt='';
		$campos=explode(",","id_marca,titulo");
			...
			<fieldset>
				...
				<div>
					<dl>
						<dt>Marca</dt>
						<dd>
							<select name="id_marca" class="obg chosen">
								<option value=""></option>
								<?php
								foreach($_marcas as $x) {
								?>
								<option value="<?php echo $x->id;?>"<?php echo $x->id==$values['id_marca']?' selected':'';?>><?php echo utf8_encode($x->titulo);?></option>
								<?php	
								}
								?>
							</select>
	...
	<div class="box-registros">
		...
		<table class="tablesorter">
			...
			<tr>
				...
				<td><?php echo isset($_marcas[$x->id_marca])?utf8_encode($_marcas[$x->id_marca]->titulo):"-";?></td>
				<td>

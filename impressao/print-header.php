<?php
	...
	$sql->consult($_p."unidades","*","where id=1");
	$unidade=mysqli_fetch_object($sql->mysqry);


	$endereco = '';

	$endereco = utf8_encode($unidade->logradouro);
	$endereco .= !empty($unidade->numero)?", ".utf8_encode($unidade->numero):"";
	$endereco .= !empty($unidade->complemento)?", ".utf8_encode($unidade->complemento):"";
	$endereco .= !empty($unidade->bairro)?", ".utf8_encode($unidade->bairro):"";
	$endereco .= !empty($unidade->cidade)?", ".utf8_encode($unidade->cidade)."-".$unidade->estado:"";
?>

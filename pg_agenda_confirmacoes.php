			...
			if(!empty($data)) {
				...
				# Agendamentos marcou/faltou
					...
					$sql->consult($_p."agenda","distinct id_paciente","where data>='".date('Y-m-d')." 00:00:00' and 
																				id_paciente IN (".implode(",",$pacientesEmTratamentosIds).") and 
																				id_status IN (1,2) and lixo=0");
					if($sql->rows) {
						while($x=mysqli_fetch_object($sql->mysqry)) {
							unset($pacientesEmTratamentosIds[$x->id_paciente]);
						}
					}
...
?>
	...
	<section class="content">  
		...
		<script type="text/javascript">
			...
			var agendaTratamento = JSON.parse(`<?php echo json_encode($agendaTratamento);?>`);

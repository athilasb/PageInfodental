				...
				if(isset($_POST['acao'])) {
					...
					if($_POST['acao']=="wlib") {
						...
						if(is_object($pergunta)) {
							...
							$vSQL="id_anamnese=".$_POST['id_anamnese'].", pergunta='".utf8_decode(ucwords($str->protege($_POST['pergunta'])))."'";
						...
						//	var_dump($_POST['opcao']);die();
									...
									if(!empty($v)) {
										if($_POST['id_opcao'][$aux]>0) {
											$vSQLOpcoes['update'][$_POST['id_opcao'][$aux]]="id_formulario=$id_formulario,
																								opcao='".utf8_decode(ucwords($str->protege($v)))."',
																								lixo=0,
							...
							$vSQL="id_anamnese=".$_POST['id_anamnese'].", pergunta='".utf8_decode(ucwords($str->protege($_POST['pergunta'])))."',tipo='".$_POST['tipo']."'";
							$vSQL.=",pub='".((isset($_POST['pub']) and $_POST['pub']==1)?1:0)."'";
							...
							if(isset($_POST['opcao']) and is_array($_POST['opcao'])) {
								$vSQLOpcoes=array();
								foreach($_POST['opcao'] as $v) {
									$vSQLOpcoes[]="id_formulario=$id_formulario,opcao='".addslashes(utf8_decode(ucwords($str->protege($v))))."'";
								}
			...
			<section class="grid">
				<div class="box">
					...
					<div class="reg">
						...
						while($x=mysqli_fetch_object($sql->mysqry)) {
						?>
						<a href="pg_configuracao_anamnese.php?form=1&edita=<?php echo $x->id;?>" class="reg-group">
